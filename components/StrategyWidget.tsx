...
// MOCK DATA based on user spec (Normally fetched from n8n)
const MARKET_SIGNALS = [
    {
        ...
        change: 2.5,
        rsi: 78,
        volumeState: 'PUMPING', // PUMPING, ACCUMULATING, DISTRIBUTING
        note: 'Dòng tiền lớn nhập cuộc'
    },
    {
        symbol: 'CTR',
        price: 112000,
        change: -0.5,
        rsi: 42,
        volumeState: 'ACCUMULATING',
        note: 'Cạn cung - Tích lũy đẹp'
    },
    {
        symbol: 'FPT',
        price: 135000,
        change: 0.2,
        rsi: 85,
        volumeState: 'DISTRIBUTING',
        note: 'Cảnh báo: Tăng giá không kèm Vol'
    }
];
...
export const StrategyWidget: React.FC = () => {
    ...
    const buyAssetPrice = MARKET_SIGNALS.find(s => s.symbol === buySymbol)?.price || 
                ...
                {activeTab === 'radar' && (
                    <div className="space-y-3 overflow-y-auto custom-scrollbar pr-1">
                        {MARKET_SIGNALS.map((signal) => {
                            ...
                            if (signal.volumeState === 'PUMPING') {
                                StatusIcon = Flame;
                                statusColor = 'text-emerald-500';
                                statusBg = 'bg-emerald-500/10 border-emerald-500/20';
                            } else if (signal.volumeState === 'DISTRIBUTING') {
                                StatusIcon = AlertTriangle;
                                statusColor = 'text-rose-500';
                                statusBg = 'bg-rose-500/10 border-rose-500/20';
                            } else if (signal.volumeState === 'ACCUMULATING') {
                                StatusIcon = Anchor;
