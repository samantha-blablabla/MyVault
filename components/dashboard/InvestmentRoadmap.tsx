...
import { ArrowRight, TrendingUp, Target, AlertTriangle, CheckCircle, Lock, Lightbulb, ShoppingBag, Calculator, Calendar, Wallet, Hourglass } from 'lucide-react';
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar pr-1 relative z-10 flex flex-col pt-4 pb-2">
                    ...
                    {activeTab === 'stocks' && (
                        <div className="flex flex-col h-full animate-fade-in">
                            {/* Advisor */}
                            {advice && (
                                <div className={`mb-5 p-3 rounded-lg border flex gap-3 flex-shrink-0 ${advice.status === 'alert' ? 'bg-rose-100 border-rose-200 text-rose-700 dark:bg-rose-500/10 dark:border-rose-500/20 dark:text-rose-300' :
                                    ...
                                    <div className="space-y-3 pt-3 border-t border-zinc-200 dark:border-zinc-800">
                                        ...
                                        <div className="px-1">
                                            <input
                                                ...
                                                max={maxAvailable}
                                                step="100000"
                                                value={tempContribution || 0}
                                                onInput={(e: any) => setTempContribution(Number(e.target.value))}
                                                onChange={(e) => setTempContribution(Number(e.target.value))}
                                                className={`w-full h-2 rounded-lg appearance-none cursor-pointer touch-none relative z-20 ${riskLevel === 'danger' ? 'bg-rose-900 accent-rose-500' :
                                        ...
                                        {/* SMART ADVISOR BOX */}
                                        <div className={`p-3 rounded-xl border text-[11px] sm:text-[10px] flex items-start gap-2 leading-relaxed ${riskLevel === 'danger' ? 'bg-rose-500/10 border-rose-500/30 text-rose-400' :
