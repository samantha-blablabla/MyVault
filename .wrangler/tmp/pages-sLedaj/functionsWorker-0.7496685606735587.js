...
// api/vndirect/[[path]].js
async function onRequest2(context) {
  const url = new URL(context.request.url);
  const path = context.params.path ? context.params.path.join("/") : "";
  const targetUrl = `https://finfo-api.vndirect.com.vn/${path}${url.search}`;
  const response = await fetch(targetUrl, {
    headers: {
      "User-Agent": "Mozilla/5.0 (compatible; FinVault/1.0)",
      "Accept": "application/json"
...
// ../.wrangler/tmp/pages-sLedaj/functionsRoutes-0.35621103206047333.mjs
var routes = [
  {
